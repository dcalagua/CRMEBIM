...
export async function crearLeadManual(formData: FormData) {
  ...
  const lead = await prisma.lead.create({
    data: {
      ...
      nivelDecision: String(formData.get("nivelDecision") ?? "").trim() || null,
      telefono: normalizarTelefono(telefono),
      email: String(formData.get("email") ?? "").trim() || null,
      linkedinContacto: String(formData.get("linkedinContacto") ?? "").trim() || null,
      sector: String(formData.get("sector") ?? "").trim() || null,
      tamanoEmpresa: String(formData.get("tamanoEmpresa") ?? "").trim() || null,
  ...
  for (const fila of data) {
    ...
    const lead = await prisma.lead.create({
      data: {
        ...
        nivelDecision: fila.nivel_decision?.trim() || null,
        telefono,
        email: fila.email?.trim() || null,
        linkedinContacto: fila.linkedin?.trim() || null,
        sector: fila.sector?.trim() || null,
        tamanoEmpresa: fila.tamano_empresa?.trim() || null,
